...
function SettingsScreen({ ctx }: { ctx: AppCtx }) {
  return (
    <div className="space-y-6">
      <Header title="Settings" sub="Profile, integrations and notifications." />
      <Card>
      ...
      <Card>
        ...
        <div className="mt-3 space-y-2 text-sm">
          <Row icon={<Mail size={14} />} label="Gmail" status="Connected" />
          <Row icon={<Sparkles size={14} />} label="Browser extension" status="Installed · Chrome" />
          {ctx.plan !== "free" && <Row icon={<MessageSquare size={14} />} label="Slack" status="Not connected" cta="Connect" onClick={() => ctx.flash("Opening Slack OAuth…")} />}
        </div>
      </Card>

      <Card>
        <h4 className="text-sm font-medium text-coral">Danger zone</h4>
        <button onClick={() => ctx.flash("Account deletion requires email confirmation.")} className="mt-3 rounded-full border border-coral/30 px-4 py-2 text-sm text-coral hover:bg-coral/10">Delete account</button>
      </Card>
    </div>
  );
}
...
function BillingScreen({ ctx }: { ctx: AppCtx }) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              ...
              <h4 className="font-display text-3xl">{tier.name}</h4>
            </div>
          ...
          <div className="flex gap-2">
            ...
            {ctx.plan !== "free" && <button onClick={() => ctx.flash("Subscription will cancel at period end.")} className="rounded-full border border-coral/40 px-4 py-2 text-sm text-coral hover:bg-coral/10">Cancel</button>}
          </div>
        </div>
      </Card>
      ...
      {ctx.plan !== "free" && (
        <Card>
          ...
          <ul className="mt-3 divide-y divide-border text-sm">
            {[
              { d: "Jun 1, 2025", a: tier.priceMonthly },
              { d: "May 1, 2025", a: tier.priceMonthly },
              { d: "Apr 1, 2025", a: tier.priceMonthly },
            ].map((i) => (
              <li key={i.d} className="flex items-center justify-between py-2">
                <span>{i.d}</span>
                <span className="inline-flex items-center gap-2"><CreditCard size={12} /> Visa •• 4242</span>
                <span>${i.a}</span>
                <button onClick={() => ctx.flash("Invoice downloaded")} className="text-ink/60 hover:text-ink"><DownloadIcon size={14} /></button>
              </li>
            ))}
          </ul>
        </Card>
      ...
      {ctx.plan === "free" && (
        <Card>
          <h4 className="font-display text-2xl">Compare plans</h4>
          <p className="mt-1 text-sm text-ink/60">See exactly what each tier unlocks.</p>
          <Link to="/plan-picker" className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-ink px-4 py-2 text-sm text-cream">View plans <ArrowUpRight size={14} /></Link>
        </Card>
      )}
    </div>
  );

...
function FloatingNav() {
  ...
  const radius = useTransform(scrollY, [0, 200], ["0px", "24px"]);
  const bg = useTransform(
    scrollY,
    [0, 200],
    ["color-mix(in oklab, var(--card) 0%, transparent)", "color-mix(in oklab, var(--card) 80%, transparent)"],
  );
  ...
  return (
    ...
    >
      <motion.nav
        style={{ width, borderRadius: radius, background: bg }}
        className="mx-auto flex max-w-6xl items-center justify-between border border-border/60 px-5 py-3 shadow-[0_18px_50px_-30px_rgba(60,40,30,0.25)] backdrop-blur-xl transition-all"
      >
        ...
        <div className="flex items-center gap-2">
          <Link to="/auth/signin" className="rounded-full px-4 py-2 text-sm hover:bg-muted">
            Sign in
          </Link>
          <Link
            to="/auth/signup"
            className="inline-flex items-center gap-1.5 rounded-full bg-ink px-4 py-2 text-sm font-medium text-cream hover:opacity-95"
          >
            Start free
      ...
      <motion.div style={{ y: yTitle }} className="relative mx-auto max-w-5xl text-center">
        ...
        <Reveal delay={0.12}>
          <p className="mx-auto mt-7 max-w-2xl text-lg text-muted-foreground">
            ...
            the average user <span className="text-foreground">$100+/month</span> — for less than $8.
          </p>
        </Reveal>
        <Reveal delay={0.2}>
  ...
  return (
    <section id="features" className="relative px-6 py-28">
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-14 grid auto-rows-[minmax(180px,auto)] gap-4 lg:grid-cols-6">
          <Bento className="lg:col-span-3 lg:row-span-2" tone="peach" icon={<LayoutDashboard size={18} />} title="The dashboard you'll actually open" body="Spend, savings, ROI and overlap — gentle on the eyes, ruthless on waste.">
            <MiniBarsArt />
          </Bento>
          <Bento className="lg:col-span-3" tone="lavender" icon={<LineChart size={18} />} title="ROI Calculator" body="Hours saved × your rate − tool cost. Per tool, per month, per year.">
            <MiniLineArt />
          </Bento>
          <Bento className="lg:col-span-2" tone="mint" icon={<Wand2 size={18} />} title="Stack Optimizer" body="Cancel, pause, switch — with reasoning and exact savings.">
            <OptimizerArt />
          </Bento>
          <Bento className="lg:col-span-1" tone="rose" icon={<Radar size={18} />} title="Overlap detection" body="Spot duplicates instantly.">
            <OverlapArt />
          </Bento>
          <Bento className="lg:col-span-2" tone="peach" icon={<BellRing size={18} />} title="Smart alerts" body="Trials, renewals, idle tools, overspend forecast.">
            <AlertsArt />
          </Bento>
          <Bento className="lg:col-span-2" tone="lavender" icon={<Users size={18} />} title="Team controls" body="Roles, project allocation, shadow-AI detection.">
            <TeamArt />
          </Bento>
          <Bento className="lg:col-span-2" tone="mint" icon={<Shield size={18} />} title="Zero-knowledge" body="We never read prompts, keystrokes or clipboards. Ever.">
            <ShieldArt />
          </Bento>
          <Bento className="lg:col-span-2" tone="butter" icon={<Share2 size={18} />} title="Exfluent Score" body="0–100 stack-health rating. Shareable, addictive.">
            <ScoreArt />
          </Bento>
    ...
    <div className="mt-1 flex items-center gap-2">
      <div className="flex -space-x-2">
        {["A", "B", "C", "D"].map((n, i) => (
          <span
            key={n}
            className="grid h-8 w-8 place-items-center rounded-full border-2 border-card font-display"
            style={{
  ...
  return (
    <footer className="relative mt-12 border-t border-border/60 bg-card/40 px-6 py-16 backdrop-blur">
      <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[2fr_1fr_1fr_1fr]">
        ...
        <FooterCol title="Product" items={["Features", "Pricing", "Extension", "Changelog"]} />
